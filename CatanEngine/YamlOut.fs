    ...
    let rec toYamli (o:obj) (ind:int) (inList:bool) :string =
        ...
        let arrayToYaml (l:'a[]) (ind:int) :string=
            ...
            | _ ->
                ...
                Util.ArrayOps.arrayFoldi (fun acc v i-> acc+ (if inList && i=1 then "" else (tabs ind))+"- "+(toYamli v (ind+1) true)) init l
        let mapToYaml (m:Map<string,_>) (ind:int) :string=
            match m.Count with
            | 0 -> "{}\n"
            | _ ->
                let init=if inList then "" else "\n"
                Util.MapOps.mapFoldi (fun acc k v i -> acc+(if inList && i=0 then "" else (tabs ind))+k+": "+(toYamli v (ind+1) false)) init m
        //handle record types
